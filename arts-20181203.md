...
### 覆盖索引
假设一张表的ID是主键, k是加了索引的字段，如果需要执行这样一条语句：`SELECT ID FROM k BETWEEN 3 AND 5`, 我们可以只查询k索引的B+树，避免回表，因为在k索引的B+树上已经可以查找到ID。
覆盖索引可以有效减少搜索次数，显著提升查询性能，是一个常用的性能优化手段。

### 最左前缀原则
可以把对多个字段建立一个联合索引，比如：对身份证号字段和地址字段建立联合索引（身份证号， 地址），可以快速通过身份证号找到对应的地址。
  - 在建立联合索引的时候，索引的字段顺序该如何选择？
    第一原则是，如果用过调整顺序，可以减少一个索引，那么这个顺序往往就是需要优先考虑采用的。
  - 假设有两个字段a和b，业务逻辑上有对a字段和b字段的联合查找，也有对b字段的单独查找。这时候需要维护(a,b)联合索引，还需要维护一个单独索引。至于选择a还是选择b，考虑的原则就是空间：哪个字段比较小，就对哪个字段做单独索引。

### 索引下推
MySQL 5.6 引入的索引下推优化（index condition pushdown）可以在搜索遍历过程中对索引中的字段做判断，过虑到不满足条件的记录，减少回表次数。
比如一张表中有联合索引(name, age)。如果一条Query语句是这样的：`SELECT * FROM user WHERE name LIKE '张 %' and age=10`，InnoDB在(name, age)索引内部就会判断age是否等于10，对于age不等于10的记录，直接跳过，避免无意义的回表。
  ...
  1. [深入浅出索引（上）](https://time.geekbang.org/column/article/69236)
...
# 4. Share
  1. [Understanding Amazon EC2 terminology](https://medium.com/@kunalyadav/understanding-amazon-ec2-terminology-85be19d0af28)
     This articles talks about some Amazon EC2 terminologies.
  2. [Don’t Shave That Yak!](https://seths.blog/2005/03/dont_shave_that/?utm_source=wanqu.co&utm_campaign=Wanqu+Daily&utm_medium=website)
