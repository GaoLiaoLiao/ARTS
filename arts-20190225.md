...
-->
  2. 线程切换带来的原子性问题
    ...
    另外一个例子在[Java Language Specification - Chapter 17. Threads and Locks](https://docs.oracle.com/javase/specs/jls/se8/html/jls-17.html#jls-17.7)中有提到：
      Java在写long/double类型的数据时候，在Java programming language memory model中被当成两次单独的写操作，每次写32位。 </br>
      而这可能导致一个线程看到的高32位和低32位来自不同的写操作。因此为保证读写操作都是atomic的，我们需要为long/double加上volatile关键字。 </br>
      事实上，Java Language Specification鼓励程序员总是为64位的变量添加volatile关键字。 </br>
      ```
