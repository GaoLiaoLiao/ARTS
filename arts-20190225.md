...
# 3. Tip
This week I learn some basic knowledge of `ExecutorService`, `Callable`, `Runnable`.
The difference between Callable and Runnable is that Callable will return something while Runnable doesn't return anything.
ExecutorService can be used to execute `Callable`s and `Runnable`s, we can do so by calling `submit`, `invokeAll` and `invokeAny`.
We can also use `ScheduledThreadPoolExecutor` to schedule tasks.
