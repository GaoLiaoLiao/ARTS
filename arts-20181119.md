...
## Python: iterable vs iterator
  ...
  2. An `iterator` is an object, which is used to iterate over an `iterable` object using `__next__` method.
  ...
  6. When a for loop is executed, for statements calls iter() on the object, which is supposed to loop over. If the call succeeds, the iter call will return an iterator object. The iterator defines the __next__() method, which accesses elements of the object one at a time. The __next()__ method will raise a `StopIteration` exception, if there are no further elements available and that's when the for loop terminates.
  7. __next()__ method can be called using the next() built-in function.

## Reference
  1. [Python | Difference between iterable and iterator](https://www.geeksforgeeks.org/python-difference-iterable-iterator/)
  2. [Python __iter__() and __next__() | Converting an object into an iterator](https://www.geeksforgeeks.org/python-__iter__-__next__-converting-object-iterator/)
  3. [What exactly are iterator, iterable, and iteration?](https://stackoverflow.com/questions/9884132/what-exactly-are-iterator-iterable-and-iteration)
