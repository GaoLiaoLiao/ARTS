...
The `java.lang.Thread` class has a method `getContextClassLoader` that returns the `ContextClassLoader` for a particular thread. The context classloader is provided by the creator of the thread when loading re
